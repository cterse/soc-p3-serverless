...
print("Orders without items: {}".format(len(itemless)))
order_duration = (max(messages) - min(messages)).total_seconds()
print("Total order duration: {:.2f}s".format(order_duration))
order_durations = [(max(ms) - min(ms)).total_seconds()
                   for ms in orders.values() if len(ms) > 1]
print("Mean order duration: {:.2f}s ({:.2f})".format(
    statistics.mean(order_durations), statistics.pstdev(order_durations)))
print("Order rate: {:.2f}".format((len(orders)-len(itemless))/order_duration))
...
for i, enactment in enumerate(items.values()):
    ...
    item_messages.extend([datetime.fromisoformat(m['_time']['S'])
                          for m in enactment])
...
print("Percentage complete: {:.2f}%".format(
    len(durations) / len(items.keys()) * 100))
