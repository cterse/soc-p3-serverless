...
class Adapter:
    ...
    def send(self, message_name, parameters):
        ...
        message = self.create_Message_(self.from_, to_, message_name, parameters)
        ...
        for parameters in message.parameters:
            ...
            for message_type in self.protocol_:
                if parameters in message_type.list_param:
                    ...
                    print("Checking " + str(message_type.message) + " for parameter " + parameters)


                    #intersection = set(enumerate(message_type.list_param)) & set(enumerate(message.parameters.keys()))

                    intersection = [p for p in message_type.list_param if p in message.parameters.keys()]
                    ...
                    query = "SELECT {columns} from {message_name} WHERE {condition};".format(
                        ...
                        condition = " and ".join(["{}='{}'".format(p,message.parameters[p]) for p in intersection])
                    )
                    ...
                    if result:
                        prior = {p : result[i] for i,p in enumerate(intersection)}
            ...
            print("Known: " + str(known[parameters]) + " for parameter " + parameters)
            if not known[parameters] and parameters in self.schema.in_param:
                print("Known: " + str(known[parameters]) + "; parameters: " + parameters + "; in-param of protocol: " + str(self.schema.in_param))
